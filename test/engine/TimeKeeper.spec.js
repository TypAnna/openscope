...
ava('#deltaTime returns a max value of 100 * TIME_UNIT', (t) => {
    TimeKeeper._frameDeltaTime = 33 * TimeKeeper.TIME_UNIT;
    TimeKeeper._simulationRate = 10;

    t.true(TimeKeeper.deltaTime === 100 * TimeKeeper.TIME_UNIT);
});
...
ava('.update() resets #_frameStartTimestamp to #currentTime when elapsed time is > frameDelay', (t) => {
    TimeKeeper._frameStartTimestamp = 10 * TimeKeeper.TIME_UNIT;
    TimeKeeper.update();
...
ava.serial('._isReturningFromPauseAndNotFutureTrack() returns false when #_frameDeltaTime is not > than 1 * TIME_UNIT and #_simulationRate is 1', (t) => {
    TimeKeeper._frameDeltaTime = 0.5 * TimeKeeper.TIME_UNIT;
    TimeKeeper._simulationRate = 1;
...
ava.serial('._isReturningFromPauseAndNotFutureTrack() returns false #_simulationRate is not === 1', (t) => {
    TimeKeeper._frameDeltaTime = 1 * TimeKeeper.TIME_UNIT;
    TimeKeeper._simulationRate = 2;
    TimeKeeper._futureTrackDeltaTimeCache = -1;
...
ava.serial('._isReturningFromPauseAndNotFutureTrack() returns true only when all three conditions are met', (t) => {
    TimeKeeper._frameDeltaTime = 2 * TimeKeeper.TIME_UNIT;
    TimeKeeper._simulationRate = 1;
    TimeKeeper._futureTrackDeltaTimeCache = -1;
